function m2d = mean2dtrj(expmat,mind,colnum,TrjAll,cutoff)
%mean2dtrj
% m2d = mean2dtrj(expmat,mind,colnum,TrjAll,cutoff)
% averages the variable given as colnum in the 2D matrix expmat
% if there are no indexes in any element of mind a NaN will be assigned
% Trajectories whose average is less than or above the cutoff will not be included 
% in the average. The cutoff can be a single value or a vector containing 
% lower and higher bounds. In each bin, first, trajectory average will be calculated
    ...
    for i = 1:length(mind(:))
        if isempty(mind{i})
            ...
            for tn = trjnums'
                ...
                % remove Nans
                vart(isnan(vart))=[];
                vart = mean(vart); % calculate the mean for this track
                % remove the value if the trajectory average if aout the
                % bounds
                if isnumeric(cutoff)
                    if vart<=cutoff(1)
                        vart = [];
