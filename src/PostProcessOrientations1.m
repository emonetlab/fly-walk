function f = PostProcessOrientations1(f)
% some parameters
...
%                 setPoint = activeFrames(randi(length(activeFrames),1));
                disp('None of the searched frmaes produced a relibale fly gradient intensity')
            ...
            orientation(ConsStart) = AllignOrient2Ref(orientation(ConsStart),meanTheta,angle_flip_threshold);
            
            % set and propogate this orientation to rest of the frames
            orientation = SetNPropOrientation(orientation,ConsStart,angle_flip_threshold);
        end
        ...
        orientation = setOrientationNPropSegments1(orientation,heading,fs,fe,frameWmaxSpeed,usefullFrames,angle_flip_threshold);
