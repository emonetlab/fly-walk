function m2d = mode2dtrj(expmat,mind,colnum,TrjAll,cutoff)
% mode averages the variable given as colnum in the 2D matrix expmat
    ...
    for i = 1:length(mind(:))
        if isempty(mind{i})
            ...
            for tn = trjnums'
                ...
                % remove Nans
                vart(isnan(vart))=[];
                vart = mode(vart); % calculate the mode for this track
                if isnumeric(cutoff)
                    if vart<=cutoff
                        vart = [];
                    end
