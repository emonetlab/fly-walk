function ref = PredRefGeom(fly,p)
%PredRefGeom
...
if isstruct(fly)
    ...
    for flynum = 1:(numel(fly))
        ...
        fly(flynum).RMaj = confac.*fly(flynum).MajorAxisLength;
        fly(flynum).RMin = confac.*fly(flynum).MinorAxisLength;
